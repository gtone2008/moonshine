...
namespace moonshine
{
    ...
    {
        string str1 = "SELECT bom_standard.bs_name, bom_standard.basic_id, description,bom_standard.bs_qty,basic_data.price,bs_qty * price as amount from bom_standard left join basic_data on bom_standard.basic_id=basic_data.basic_id ";
        string str11 = "insert into bom_standard(bs_name, basic_id, bs_qty) values ('{0}','{1}',{2})";//加入BOM
        string str2 = "SELECT description,spec FROM basic_data where 1=1 and basic_id='{0}' ";// //下拉菜单PN号取描述
        string str21 = "SELECT ps_name FROM product_standard where 1=1 and ps_id='{0}' ";// //下拉菜单Item号取name
        string str3 = "SELECT basic_id FROM basic_data  "; //下拉菜单PN号
        string str4 = "SELECT ps_id FROM product_standard  "; //下拉菜单item号
        string str5 = "delete from bom_standard ";
