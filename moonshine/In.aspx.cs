...
namespace moonshine
{
    ...
    {
        string str1 = "SELECT in_id,description,spec,current_qty,in_qty,type,reques_by,remark,in_date FROM in_data left join basic_data on in_data.in_id=basic_data.basic_id where 1=1 ";
        //string str2 = "SELECT description,spec,current_qty FROM basic_data ";
        //string str3 = "SELECT basic_id FROM basic_data  ";
        string str4 = "INSERT INTO  in_data(in_id, in_qty, type, reques_by, remark) values('{0}',{1},'{2}','{3}','{4}');";
        ...
        protected void btnInsert_Click(object sender, EventArgs e)
        {
            ...
            str4 = string.Format(str4, Request.Form["pn1"].Trim(), int.Parse(Request.Form["txtqty"].Trim()), ddltype.SelectedValue, txtreq.Text.Trim(), txtrem.Text.Trim());
            ...
            if (MysqlHelper.ExecuteNonQuery(str4) > 0)
            {
                if (MysqlHelper.ExecuteNonQuery(str5)>0)
                {
                    ...
                    Response.Redirect("In.aspx");//防止重复提交
                    //dvband();
                }
